...
export function publishToRoom(janus, opaqueId, room, secret, 
    ...
    let sfutest = null;
    let mystream = null;
    console.log("inside publishToRoom", room)
    ...
    janus.attach(
        {
            ...
            success: function (pluginHandle) {
                ...
                if(isPublisher){    
                    console.log("Inside publisher condition to create room: ", room, "typer - ", typeof room)
                    ...
                    // Send message to check if the room exists
                    sfutest.send({
                        message: checkRoom,
                        success: function(response) {
                            if (response.exists) {
                                // Room exists, send delete request
                                const deleteRoom = {
                                    "request": "destroy",
                                    "room": Number(room),
                                    "secret": secret
                                ...
                                sfutest.send({ message: deleteRoom });
                                setTimeout(() => {
                                    console.log("4 seconds passed, proceeding...");
                                }, 4000);
                            } else {
                                console.log("Room does not exist, skipping delete request.");
                            }
                        },
